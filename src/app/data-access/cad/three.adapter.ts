...
export interface ManifoldBridge {
  manifoldToThree(m: any, material: THREE.Material): THREE.Mesh; // Manifold → THREE.Mesh
  threeToManifold(mesh: THREE.Mesh): any;                         // THREE.Mesh → Manifold
}
...
export class ThreeAdapter {
  ...
  constructor(opts?: { exporter?: STLExporter; loader?: STLLoader; manifold?: ManifoldBridge }) {
  ...
  meshesFromGeom3(geom: Geom3 | Geom3[], options: MeshFromGeomOptions = {}): THREE.Mesh[] {
  ...
  meshFromManifold(m: any, material?: THREE.Material): THREE.Mesh {
    if (!this.manifold) throw new Error('ThreeAdapter: no ManifoldBridge provided.');
  ...
  manifoldFromThree(mesh: THREE.Mesh): any {
    if (!this.manifold) throw new Error('ThreeAdapter: no ManifoldBridge provided.');
...
function debugFaceColorMaterial(geom: THREE.BufferGeometry): THREE.Material {
  ...
  return new THREE.MeshBasicMaterial({ vertexColors: true, transparent: true, opacity: 0.65 });
}
